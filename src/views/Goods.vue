<template>
  <div class="goods" id="goods" name="goods">
    ...
    <div class="btarea1">
      <div class="btarea2">
        <div class="btarea3">
          <el-button :class="{ active: isPrice }" class="sortButton" @click="priceSort"
            >价格
            <span v-show="sortAsPrice == -1">↓</span>
            <span v-show="sortAsPrice == 1">↑</span>
          </el-button>
          <el-button :class="{ active: isSales }" class="sortButton" @click="salesSort"
            >销量
            <span v-show="sortAsSales == -1">↓</span>
            <span v-show="sortAsSales == 1">↑</span>
          </el-button>
        </div>
      </div>
...
export default {
  ...
  activated() {
    // this.username = this.getOne.userName;
  ...
  computed: {
    isPrice() {
      return this.sortAsPrice != 0;
    },
    isSales() {
      return this.sortAsSales != 0;
    },
  },
  methods: {
    // 返回顶部
    ...
    getData() {
      ...
      this.$axios
        ...
        .then((res) => {
          ...
          this.productBackUp = JSON.parse(JSON.stringify(this.product));
        })
        .catch((err) => {
          return Promise.reject(err);
        });
